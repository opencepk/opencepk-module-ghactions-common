...
module.exports.create = (
  ...
  return app.connect(timeout, proxy).then(() => {
    return app;
  });
};
...
class GitHubApplication {
  constructor(privateKey, applicationId, baseApiUrl) {
    this._config = {
      privateKey: new PrivateKey(
        _validateVariableValue('privateKey', privateKey),
      ),
  ...
  getApplicationInstallations() {
    return this.client
      ...
      .catch(err => {
        throw new Error(
          `Failed to get application installations; ${err.message}`,
        );
      })
      .then(resp => {
        if (resp.status === 200) {
  ...
  getInstallationAccessToken(installationId, permissions) {
    ...
    return this.client
      ...
      .catch(err => {
        throw new Error(
          `Failed to get access token for application installation; ${err.message}`,
        );
      })
      .then(resp => {
        if (resp.status === 201) {
