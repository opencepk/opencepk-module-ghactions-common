...
class GitHubApplication {
  constructor(privateKey, applicationId, baseApiUrl) {
    this.privateKey = new PrivateKey(validateInput('privateKey', privateKey)).key;
    this.id = validateInput('applicationId', applicationId);
    this.githubApiUrl = baseApiUrl || process.env['GITHUB_API_URL'] || 'https://api.github.com';
    this.client = null;
  }
  ...
  async connect(validSeconds = 60, proxy) {
    ...
    try {
      const resp = await this.client.request('GET /app', { mediaType: { previews: ['machine-man'] } });
      if (resp.status === 200) {
  ...
  async getRepositoryInstallation(owner, repo) {
    return this._request('GET /repos/{owner}/{repo}/installation', { owner, repo });
  }
  ...
  async getInstallationAccessToken(installationId, permissions = {}) {
    if (!installationId) throw new Error('GitHub Application installation id must be provided');
    return this._request(`POST /app/installations/${installationId}/access_tokens`, { permissions });
  }

  async _request(endpoint, params = {}) {
    try {
      const resp = await this.client.request(endpoint, { mediaType: { previews: ['machine-man'] }, ...params });
...
function validateInput(name, value) {
  if (!value || !value.trim()) throw new Error(`A valid ${name} must be provided, was "${value}"`);
  return value.trim();
}
